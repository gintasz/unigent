...
- **A1. Architectural style** `[tradeoff]` — Modular monorepo. The reusable core library and the harness-agnostic **runtime** (the turn-executing component that runs the repair loop, caps, and dispatch) never depend on any specific agent harness; a harness is an adapter built on top of them. *(review)*
...
- **L2. Paradigm — plain TypeScript, Promise at the seam** `[tradeoff]` — The core is plain, strict TypeScript. Domain logic (config cascade, usage accounting, schema validation, control-tool dispatch) is expressed as pure values and functions; asynchrony, cancellation, and timeouts are handled with `Promise`/`async` and `AbortSignal` at the program edge / public facade (F6). Immutability and composition-over-inheritance hold; classes are used for genuinely stateful adapters and the public API surface; mutation is the localized exception (S2). No additional functional-runtime substrate is layered over the language. *(judgment)*
- **L3. Banned constructs** `[consistency]` — `export *` from package barrels; deep cross-package imports; `any`; non-null `!` assertions outside a justified local; free-floating module-global mutable state (A6); and stringly-typed domain failures or per-function ad-hoc `{ ok }`/`Result` shapes used in place of the one error taxonomy (T6/F7) or a discriminated union (T3). *(tool: linter + custom code-pattern checker + import-graph checker + review)* — the ad-hoc `{ ok }`/`Result`-shape and stringly-typed-failure bans are review-enforced (no current tool checks them); the remainder is tool-enforced.
...
- **F2. Structured control, never string-matched.** The agent affects the program only through a fixed, small set of structured **control tools** surfaced as native function-calling (the four reserved `foom_*` tools — the codebase calls these "control tools" everywhere; "FOOM tools" is an alias for the same set); their effect is never parsed out of free-text output. Prompt prose is unconstrained; control is structured. *(review)*
...
- **I1. Contract source of truth** `[tradeoff]` — Code-first and single-sourced: the protocol surface (control-tool definitions, prompts, operator commands) is defined once in the generic layer; harnesses consume it, never redefine it. *(review)*
