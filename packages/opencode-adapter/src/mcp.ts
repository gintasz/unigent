...
// closure over the live program state — and the OpenCode server (a child process)
...
//
// Only the minimal JSON-RPC subset OpenCode drives is implemented: initialize,
// tools/list, tools/call (verified against the real binary). OpenCode opens a GET
// SSE stream on connect; we acknowledge it with 202 and it falls back to plain
// request/response. Tools are registered under their CANONICAL basenames (OpenCode
// adds the `<server>_` prefix itself, and strips it before calling); only the
// description text is renamed so its references match what the model sees.
...
/** The protocol version we echo back; OpenCode negotiates from its own. */
 ...
 *  model-native field an MCP tool has — its description. (OpenCode MCP tools carry
 *  no separate promptSnippet slot.) */
...
interface McpServerHandle {
  /** The URL OpenCode is pointed at via the `mcp` config. */
  readonly url: string;
...
async function startMcpServer(
  ...
  const server: Server = createServer((req: IncomingMessage, res: ServerResponse) => {
    (async () => {
      // Only POST carries JSON-RPC; GET (the SSE open) / DELETE / empty bodies are
